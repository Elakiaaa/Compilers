...
int getKnownValue(KNOWN* kVar,char* id){
	KNOWN* temp = kVar;
	if(strcmp(id,"") == 0) return 0;
	while(temp != NULL){
		if(strcmp(temp->id,id)==0) return temp->value;
		temp = temp->next;
	}
	return -1;

}
KNOWN* getKnode(char* id,int val){
	KNOWN* new = (KNOWN*)malloc(sizeof(KNOWN));
	new->id = id;
	new->value= val;
	new->next = NULL;
}
void constantFold(int first , int last){
	KNOWN* kVar = NULL;
	int i=first;
	for(; i<= last ; i++){
		int v1,v2;
		QUAD q = instrArr[i];
		/*if(isnumber(q.arg1) || getKnownValue(kVar,q.arg1) != -1)
			printf("n %s\n",q.arg1);
		else
			printf("nan %s\n",q.arg1);

		if(isnumber(q.arg2) || getKnownValue(kVar,q.arg2)!= -1)
			printf("n %s\n",q.arg2);
		else
			printf("nan %s\n",q.arg2);*/
		
		if(isnumber(q.arg1) == 0 && getKnownValue(kVar,q.arg1) == -1 ) continue;
		if(isnumber(q.arg2) == 0 && getKnownValue(kVar,q.arg2) == -1 ) continue;

		if(isnumber(q.arg1)) v1 = atoi(q.arg1);
		else v1 = getKnownValue(kVar,q.arg1);

		if(isnumber(q.arg2)) v2 = atoi(q.arg2);
		else v2 = getKnownValue(kVar,q.arg2);
		//printf("%d  %d\n",v1,v2);

		int ans;
		if(strcmp(q.op,"*")==0) ans=v1*v2;
		else if(strcmp(q.op,"-")==0) ans=v1-v2;
		else if(strcmp(q.op,"/")==0) ans=v1/v2;
		else if(strcmp(q.op,"*")==0) ans=v1*v2;
		else if(strcmp(q.op,"%")==0) ans=v1%v2;
		else  ans = v1+v2;
		...
		char res[100];
		sprintf(res,"%d",ans);
		instrArr[i].arg2 = strdup(res);
		//printf("%s\n",instrArr[i].arg1);
		instrArr[i].arg1 = "";
		instrArr[i].op = "=";

		KNOWN* new = getKnode(q.result,ans);
		new->next = kVar;
		kVar = new;	
